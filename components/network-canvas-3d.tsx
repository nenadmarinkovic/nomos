...
/**
 * 3D companion to NetworkCanvas. Same trade-tie graph, but laid out in space
 * via d3-force-3d under the hood. Aesthetic-first; the 2D view remains the
 * one to read structure precisely.
 */
    ...
    <div ref={containerRef} className="relative flex h-full flex-1">
        ...
        {size.w > 0 && size.h > 0 ? (
          <ForceGraph3D
            ...
            nodeThreeObject={(n: any) => {
              ...
              const geom = motivationGeometry(n.motivation as string, r);
              const mat = new THREE.MeshLambertMaterial({
                color,
                emissive: n.id === selectedId ? color : 0x000000,
                emissiveIntensity: n.id === selectedId ? 0.6 : 0,
              });
      ...
      <div className="pointer-events-none absolute left-4 top-4 z-10 flex flex-col gap-1 rounded-md border border-foreground/10 bg-card/90 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-muted-foreground/70">3D</span>
        </div>
      ...
      {selectedId === null && (
        <div className="pointer-events-none absolute bottom-4 left-4 z-10 max-w-md rounded-md border border-foreground/10 bg-card/90 px-3 py-2 font-serif text-[12px] italic leading-snug text-foreground/80 backdrop-blur-sm">
          Drag to orbit. Scroll to zoom. Click an agent to focus the camera and
          inspect — click empty space to release.
        </div>
      )}
