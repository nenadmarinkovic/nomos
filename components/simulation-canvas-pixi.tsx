...
interface SimulationCanvasPixiProps {
  running: boolean;
}
...
/** Phase-A Pixi renderer. Bootstraps a WebGL Application, draws every alive
 *  agent as a motivation-coloured sprite, interpolates positions between
 *  ticks. Resource grid, selection ring, hit-tests are not yet ported —
 *  those live in `simulation-canvas.tsx` (Canvas2D) until phases C and D. */
export function SimulationCanvasPixi({ running }: SimulationCanvasPixiProps) {
  ...
  const paint = useCallback((progress = 1) => {
    ...
    const resourceTexture = resourceTextureRef.current;
    const resourceSprite = resourceSpriteRef.current;
    if (resourceCanvas && resourceTexture && resourceSprite) {
  ...
  useEffect(() => {
    ...
    void app
      ...
      .then(() => {
        ...
        resourceCanvas.width = 1;
        resourceCanvas.height = 1;
        const resourceTexture = Texture.from(resourceCanvas);
      ...
      <div ref={containerRef} className="relative flex-1 overflow-hidden">
        ...
        <div className="absolute right-3 top-3 flex items-center gap-2 rounded-md border border-border bg-card/80 px-2 py-1 backdrop-blur-sm">
          ...
          <span className="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">
            {running ? "running · pixi" : started ? "paused · pixi" : "idle"}
          </span>
        </div>
      </div>
    </div>
